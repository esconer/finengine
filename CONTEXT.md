...
13. **Stress Testing Volatility Factor & Padding Immunity**: When computing volatility scaling factors ($\text{VolFactor}_i = \sigma_i / \sigma_{\text{market}}$) for multi-year historical stress tests across portfolios with mixed listing histories, calculate $\sigma_i$ on active non-zero returns (`s[s != 0.0]`). Zero-padded arrays artificially compress sample standard deviation down to market volatility levels, causing high-beta smallcaps to falsely render uniform benchmark drawdown impacts.
