...
| **Data Vendors** | User-selectable primary: `bfinance` (10-13Y Ind AS, concalls, fast quotes, default Tier 1) or `yfinance` (.NS/.BO auto-suffix) — the other becomes the fallback; Alpha Vantage (rotating key pool) is ALWAYS the last tier. Persisted in `app_settings`, honored per-fetch. | Cached in SQLite `stock_timeseries` |
...
- **User-Selectable Data Source Cascade**: The primary market-data vendor (`bfinance` or `yfinance`) is chosen in Settings (`PUT /data/config`, persisted in `app_settings`); the other vendor is the fallback and Alpha Vantage is always last. Applies to OHLCV, quotes, ticker validation, and fundamentals per-fetch. Screener/Equity-Research/AI-Dossier stay bfinance-exclusive (no yfinance equivalent); USD/INR FX stays yfinance-only. `POST /data/cache/clear` purges all cached market data + in-process memo caches while never touching `portfolio_positions` (holdings truth).
...
21. **Concurrent Workers Sharing One AsyncSession Need a DB Gate**: SQLAlchemy `AsyncSession` is not concurrency-safe — parallel commits/rollbacks on a shared session fail with `commit() can't be called here`, `transaction is closed`, or `session is provisioning a new connection` (observed as 321 errors / 145 dropped stores in one 20s portfolio refresh). `DataService` serializes every session-touching section behind an instance-level `asyncio.Lock` (`_db_lock`) while keeping vendor network calls parallel. Batch helpers must also resolve config reads (e.g. the data-source preference) ONCE per batch and thread results down (`fetch_historical_data(..., source_order=)`). Regression guard: `tests/test_db_gate_concurrency.py` (OverlapGuardSession flags any overlapping DB op). **Second layer**: the dashboard fires many parallel *requests*, each with its own session — cross-session SQLite write contention requires `PRAGMA busy_timeout` (30s) + WAL. **aiosqlite trap**: connect-event pragmas receive SQLAlchemy's `AsyncAdapt_aiosqlite_connection`, NOT `sqlite3.Connection` — an `isinstance(dbapi_connection, sqlite3.Connection)` guard silently disables the entire pragma block (WAL/busy_timeout never applied for months); execute pragmas directly through the adapter's sync `execute()`.
22. **Pandas Series Values in DataFrame Constructors Align Against the Explicit Index**: Building a vendor-style OHLCV frame with `pd.Series` column values plus an explicit `DatetimeIndex(index=...)` silently aligns the Series' RangeIndex against the dates, yielding all-NaN columns that `dropna()` empties (0-row frames that still "succeed"). Use numpy arrays in test fixtures and ingestion boundaries.
23. **Two-Window Risk Split & Degenerate Fallbacks**: Realized P&L is holding-truthed (masked to ownership start); instrument risk characteristics (vol, Sharpe, beta, covariance, drawdown) are ASSET properties and must compute on full exchange history — `get_realized_risk.instrument_risk` + `get_factor_exposure` follow this split (DSP-10). Masking a regression window to ~6 observations degenerates it, and engines must never render placeholder fallbacks (`_empty_factor_exposure`'s `{alpha:0, market:1}` showed as "β +1.000 Market-Like" for every position) — return N/A + a flag, or compute on the full window. Also: `series or []` on a pandas Series raises "truth value is ambiguous" — use explicit `is not None` checks.
24. **Canonical Quant Formulas (corrected 2026-09-07, see BACKEND_REVIEW §5)**: Sortino = downside deviation `sqrt(mean(min(0,r-target)²))·√252` (not `std(negatives)`); EWMA = single-pass RiskMetrics `σ²=λσ²+(1-λ)r²` with flat term structure (no rolling-std pre-smooth); CVaR `alpha` is a free `cp.Variable()` (VaR of losses can be positive); backtest turnover is one-way `0.5·Σ|Δw|` with multiplicative day-0 cost, Sharpe mean-based `ddof=1`; OU `-2<γ≤-1` is oscillatory → `(None,None)`, never a hardcoded half-life. Regressions guarded by `tests/test_quant_math_p1_batch.py`.
